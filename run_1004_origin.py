...
def Run_EVSGL(args):
    ...
    for i in range(numView):
        ...
        for epoch in range(args.gmim_epochs):
            ...
            TMIM_Loss = norm * F.binary_cross_entropy_with_logits(
                recons_graph.reshape(-1),
                raw_adj.reshape(-1),
    ...
    for epoch in range(args.fusion_epochs):
        ...
        for i in range(numView):
            ...
            fusion_loss += norm * F.binary_cross_entropy_with_logits(recons_graphs[i].reshape(-1), Raw_adj_list[i].reshape(-1), pos_weight=pos_weight)
    ...
    for epoch in range(args.finetune_epochs):
        ...
        for i in range(numView):
            ...
            recons_loss += (norm * F.binary_cross_entropy_with_logits(recons_graphs[i].reshape(-1), Raw_adj_list[i].reshape(-1), pos_weight=pos_weight))
